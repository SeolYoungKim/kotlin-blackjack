...
블랙잭 게임은 딜러와 겜블러 중 카드의 합이 21 또는 21에 가장 가까운 숫자를 가지는 쪽이 이기는 게임이다.

- 카드의 숫자 계산은 카드 숫자를 기본으로 하며, 예외로 Ace는 1 또는 11로 계산할 수 있으며, King, Queen, Jack은 각각 10으로 계산한다.
- 게임을 시작하면 겜블러는 두 장의 카드를 지급 받으며, 두 장의 카드 숫자를 합쳐 21을 초과하지 않으면서 21에 가깝게 만들면 이긴다. 
...
- 블랙잭 게임 룰
  ...
  - 겜블러의 카드 합은 21을 초과하지 않아야 한다. (초과 시 패배? 게임을 종료해야 하나?)
    - 21을 넘지 않을 경우, 얼마든지 카드를 뽑을 수 있다. 
  - 21을 초과하지 않으면서, 결과가 21에 가까운 쪽을 선택하는 것이 유리하다.
  - Ace 점수는 1 또는 11 중에서 겜블러가 자유롭게 선택할 수 있다.
...
- 딜러가 21을 초과하면 그 시점까지 남아 있던 겜블러들은 가지고 있는 패에 상관 없이 승리한다.
- 게임을 완료한 후 각 겜블러별로 승패를 출력한다.

### 프로그래밍 요구사항 
- 3개 이상의 인스턴스 변수를 가진 클래스를 쓰지 않는다.
- 딜러와 겜블러에서 발생하는 중복 코드를 제거해야 한다.
...
- [x] 딜러 카드가 21을 초과한 경우 겜블러들이 이김  


## 4단계 
### 기능 요구사항 
- 겜블러는 게임을 시작할 때 베팅 금액을 정해야 한다.
- 카드를 추가로 뽑아 21을 초과할 경우 베팅 금액을 모두 잃게 된다.
- 처음 두 장의 카드 합이 21일 경우 블랙잭이 되면 베팅 금액의 1.5 배를 딜러에게 받는다.
- 딜러와 겜블러가 모두 동시에 블랙잭인 경우 겜블러는 베팅한 금액을 돌려받는다.
- 딜러가 21을 초과하면 그 시점까지 남아 있던 겜블러들은 가지고 있는 패에 상관 없이 승리해 베팅 금액을 받는다.

### 기능 목록 
#### 출력 
- [x] 배팅 금액 입력 안내 메세지 출력 
...
- [x] 겜블러들은 배팅을 할 수 있다 
- 배팅 금액 정산
  - [] 무승부인 경우 겜블러가 배팅 금액을 돌려받는다
  - [] 겜블러가 이기는 경우 배팅 금액을 받는다 
    - [] 딜러가 21을 초과하면 겜블러들이 승리하므로 겜블러들은 배팅 금액을 돌려받는다
    - [] 딜러가 21점 이하이고 겜블러가 딜러보다 점수가 높으면 승리하므로 배팅 금액을 돌려받는다 
    - [] 겜블러의 카드가 2장이고 블랙잭(합이 21)인 경우 1.5배 받는다  
